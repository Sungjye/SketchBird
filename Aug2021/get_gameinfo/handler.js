...
module.exports.GetGameInfo = async (event) => {
  ...
  for(var idx=0; idx < intNumOfFinishedUsers; idx++)
  {
    ...
    if( intNumOfFinishedUsers == (idx+1) ) // #종료게임용 별도 처리필요!
    {
      ...
      intCurrentTurn = queryResult[idx].turnNumber + 1; 
  ...
  */
  const strNextUserUid = queryResult[0].userUid;
  // intNextUserIdx
  const strNextUserNickname = queryResult[0].nickName;
  const intNextUserGender = queryResult[0].gender;
  const intNextUserAgeRange = queryResult[0].ageRange;
  const strNextUserPushToken = queryResult[0].fcmToken;
  const strNextUserCountry = queryResult[0].region;
  const strNextUserUserLanguage = queryResult[0].userLanguage;
  // intNextUserTurnNum
  // strNextUserDrawType
