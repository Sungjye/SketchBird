...
// 
// 2021.12.27. sjjo. Unseen 테이블도 조인해서 그 결과를 내 게임 리스트 보내줄 때 같이 주기. 
//                   조인이 어려움. 유니온 등을 해야할 것 같은데, 쿼리 한번더 해야하므로.. 일단 사용하지 않음. 
//
//-------------------------------------------------------------------------------------
// References.
//
...
async function queryThisFromTheTable( query_str ){
  return new Promise( (resolve, reject) => {
    ...
    pool.query(query_str, function(err, result, fields) {
      ...
      if(err)
      {
          const errMsg = "[GetMyGameListV3] [ERROR]: db-error:" + err;
          console.log(errMsg);
          reject(errMsg);
      }
...
module.exports.GetMyGameListV3 = async (event) => {
  ...
  {
    const err_msg = "[GetMyGameListV3] [Serious Error!!] Invalid Request Data! event: " + JSON.stringify(event);
    console.log( err_msg );
  ...
  */                    
  ...
  }catch(error)
  {
    ...
    const err_msg ="[GetMyGameListV3] [Error] [Ing Game] SELECT FAIL. DB Access or etc.: " 
  ...
  */
  ...
  {
    const err_msg ="[GetMyGameListV3] [Error] [Finished Game] SELECT FAIL. DB Access or etc.: " 
            + "event: " + JSON.stringify(event)
            + " > " + error;
    console.log( err_msg );                    
    return sendRes(400, err_msg); 
  ...
  //-------------------------------------------------------------------------
    const err_msg ="[GetMyGameListV3] [Error] Not supported GameStatus type! Ask to JO! : "
          + "event: " + JSON.stringify(event);
    console.log( err_msg );                    
