...
//
// 2021.12.28. sjjo. mysql2 pool 방식 적용. 이제 정말 사용할 시점을 앞둔듯... OMLJ, TWBTGT ITEOY
//
//-------------------------------------------------------------------------------------
...
const { send } = require('process'); // 엇다 쓰는 거였지? ㅡ_ㅜ
...
async function queryThisFromTheTable( query_str ){
  return new Promise( (resolve, reject) => {
    ...
    pool.query(query_str, function(err, result, fields) {
      ...
      if(err)
      {
          const errMsg = "[GetRandomUserV2] [ERROR]: db-error:" + err;
          console.log(errMsg);
          reject(errMsg);
      }
...
module.exports.GetRandomUserV2 = async (event) => {
  ...
  if( strDrawType == 'draw' ) 
  {
    ...
    try{
      ...
      */
      ...
      {
        //const err_msg = "[GetRandomUserV2] [Error] [UserTable] There is no random user! numOfUser: " + queryResult.length ;
        //console.log( err_msg );
    ...
    {
      //console.log("[GetRandomUserV2] [Error] [UserTable] SELECT FAIL. DB Access or.. : " + " > " + error);
      ...
      {
        strErrMsg = "[GetRandomUserV2] [Error] [UserTable] There is no random user!";
        console.log(strErrMsg);
        return sendRes(406, strErrMsg);

      }else
      {
        strErrMsg = "[GetRandomUserV2] [Error] [UserTable] SELECT FAIL. DB Access or.. : " + " > " + error;
        console.log(strErrMsg);
  ...
  // TBD. 언어 여러개 놓고, 테스트 해서 확인해 해야 함. 2021.10.07
    ...
    try{
      ...
      if( queryResult.length != 1 ) 
      {
        //const err_msg = "[GetRandomUserV2] [Error] [UserTable] There is no random user with this language: " + strUserLanguage ;
        //console.log( err_msg );
    ...
    }catch(error)
    {
      ...
      {
        strErrMsg = "[GetRandomUserV2] [Error] [UserTable] There is no random user with this language: " + event.userLanguage ;
        console.log(strErrMsg);
