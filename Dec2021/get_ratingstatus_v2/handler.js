...
// 
// 2021.12.15. sjjo. 기존의 v1을 v2로 변경. pool 방식으로 변경.
// 2021.12.15. sjjo. 스코어 값까지 스코어카운트테이블에서 가져와서 같이 리스폰스. 
//
//-------------------------------------------------------------------------------------
'use strict';
...
module.exports.GetRatingStatusV2 = async (event) => {
  ...
  try{
    ...
    /* 
    SELECT R.userIdx, R.gameIdx, R.checkedFun, R.checkedGoldhand, R.checkedReport, S.funScore, S.goldHandScore FROM RatingStatusTable AS R 
					INNER JOIN ScoreCountTable AS S ON R.gameIdx=S.gameIdx 
                    WHERE R.userIdx=15 AND R.gameIdx=206;
    */
    //const query_ratingStatusInfo = 'SELECT * FROM RatingStatusTable WHERE userIdx=' + intUserIdx + ' AND gameIdx=' + intGameIdx + ';';
    const query_ratingStatusInfo = 'SELECT R.userIdx, R.gameIdx, R.checkedFun, R.checkedGoldhand, R.checkedReport, S.funScore, S.goldHandScore FROM RatingStatusTable AS R '
                                            + 'INNER JOIN ScoreCountTable AS S ON R.gameIdx=S.gameIdx '
                                            + 'WHERE R.userIdx=' + intUserIdx 
                                            + ' ' + 'AND R.gameIdx=' + intGameIdx + ';';
  ...
  if( queryResult.length == 1)
  {
    ...
    */
    const response_data = {
      ...
      funScore: queryResult[0].funScore,
      goldHandScore: queryResult[0].goldHandScore
    };
