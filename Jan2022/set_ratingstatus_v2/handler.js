...
//
// 2022.01.19. sjjo. 앱연동 시작하면서 버그 수정. 업데이트 테이블 쪽.
//-------------------------------------------------------------------------------------
...
async function queryThisFromTheTable( query_str ){
  return new Promise( (resolve, reject) => {
    ...
    pool.query(query_str, function(err, result, fields) {
      ...
      if(err)
      {
          const errMsg = "[SetRatingStatus] [ERROR in query function]: db-error:" + err;
          console.log(errMsg);
          reject(errMsg);
      }
...
module.exports.SetRatingStatusV2 = async (event) => {
...
//-------------------------------------------------------------------------
  ...
  console.logDebug(query_upsertRating);      
...
ResultSetHeader {
  ...
  */
  ...
  //if(queryResult.affectedRows == 1 ) // 2021.12.15 정상적인 경우 추려내기. 
  if(queryResult.warningStatus == 0 ) // 2021.12.15 정상적인 경우 추려내기. 
  {
  ...
  // UPDATE users SET reviews_len = reviews_len + 1 WHERE user_id = 1
    ...
    try{
      ...
      console.logDebug(query_EndGame_ScoreCountTable); // 2022.01.19.
