...
module.exports.UpdateFcmToken = async (event) => {
  ...
  try{
    ...
    }else
    {
      ...
      return sendErr(410, "[UpdateFcmToken] [Error] token is not updated. Msg: " + updateResult.message);

    }

  }catch(error){

    const strErrorInfo = "[UpdateFcmToken] [Error][DB access Failed]: " + " > " + error;
    console.log(strErrorInfo);
    // return sendRes(400, error);
    return sendErr(400, strErrorInfo);
