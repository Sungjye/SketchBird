...
//const CONST_NUMOF_FULLSET_PARAM = 19; // (인서트를 하기위한 조건인) 사용자 정보 플세트 파라메터의 개수.

const ERR_NO_VALID_USER_TO_UPDATE = 1;
const ERR_MORE_THAN_ONE_USER = 2;
...
module.exports.UpsertUserInfo = async (event) => {
...
//-------------------------------------------------------------------------
  ...
  {
    const strInfoMsg = '[UpsertUserInfo] [Error] Null data!!!: ' 
...
{
  const strInfoMsg = '[UpsertUserInfo] [Error] There is/are NULL data!!!: ' 
  +'userUid: ' + event.userUid
  +'originSNS: ' + event.originSNS;
  console.log(strInfoMsg);
...
}catch(error)
{
  ...
  {
    const strInfoMsg = "[UpsertUserInfo] [SERIOUS Error!!] [UserTable] There is/are more than one users! "
                        +'userUid: ' + event.userUid
                        +'originSNS: ' + event.originSNS;
    console.log(strInfoMsg);
  ...
  {
    // 이 경우는 
    const strInfoMsg = "[UpsertUserInfo] [Error] [UserTable] SELECT FAIL. DB Access or etc.:  " + " > " + error;
    console.log(strInfoMsg);
    return sendRes(400, strInfoMsg);      
  }
}
...
if( numOfUser == 1) // 사용자가 1명 있고, 넘어온 데이터가 다 있어서, 업데이트 하는 경우.
{
  ...
  //console.log(strUpdateUserInfo);
    ...
    {
      const strInfoMsg = "[UpsertUserInfo] [Error] [UserTable] #UPDATE# FAIL. DB Access or etc.:  " + " > " + error;
      console.log(strInfoMsg);
      return sendRes(400, strInfoMsg);     
    }
...
}else if( numOfUser == 0) // 사용자가 없고, 넘어온 데이터가 다 있어서, 인서트 하는 경우. 
{
    ...
    {
      const strInfoMsg = "[UpsertUserInfo] [Error] [UserTable] #INSERT# FAIL. DB Access or etc.:  " + " > " + error;
      console.log(strInfoMsg);
      return sendRes(400, strInfoMsg);     
    }
    ...
    {
      const strInfoMsg = "[UpsertUserInfo] [Error] [UserTable] SELECT FAIL after INSERT. DB Access or etc.:  " + " > " + error;
      console.log(strInfoMsg);
      return sendRes(400, strInfoMsg);     
    }    
