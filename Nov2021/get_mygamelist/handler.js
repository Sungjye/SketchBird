...
//
// 
// 2021.11.18. sjjo. Initial.
// 2021.11.18. sjjo. The first deploy.
// 2021.11.23. sjjo. 진행중 게임에도 (전사람이 그린 이미지 URL 추가. 프럼 프로그래스테이블)
// 
...
function queryThisFromTheTable(query_string) {
  return new Promise((resolve, reject) => {
    ...
    mysql_connection.query(query_string, function(err, result, field) {
        ...
        if(err)
          console.log("[GetMyGameList] [ERROR]: db-error:",err);
    });
...
module.exports.GetMyGameList = async (event) => {
  ...
  {
    const err_msg = "[GetMyGameList] [Serious Error!!] Invalid Request Data! event: " + JSON.stringify(event);
    console.log( err_msg );
  ...
  */                    
  ...
  }catch(error)
  {
    ...
    const err_msg ="[GetMyGameList] [Error] [Ing Game] SELECT FAIL. DB Access or etc.: " 
  ...
  */
  ...
  {
    const err_msg ="[GetMyGameList] [Error] [Finished Game] SELECT FAIL. DB Access or etc.: " 
            + "event: " + JSON.stringify(event)
            + " > " + error;
    console.log( err_msg );                    
    return sendRes(400, err_msg); 
  ...
  //-------------------------------------------------------------------------
    const err_msg ="[GetMyGameList] [Error] Not supported GameStatus type! Ask to JO! : "
          + "event: " + JSON.stringify(event);
    console.log( err_msg );                    
