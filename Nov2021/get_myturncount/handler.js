...
function queryThisFromTheTable(query_string) {
  return new Promise((resolve, reject) => {
    ...
    mysql_connection.query(query_string, function(err, result, field) {
        ...
        if(err)
          console.log("[GeyMyturnCount] [ERROR]: db-error:",err);
    });
...
module.exports.GetMyturnCount = async (event) => {
  ...
  {
    const err_msg = "[GeyMyturnCount] [Serious Error!!] Invalid Request Data! event: " + JSON.stringify(event);
    console.log( err_msg );
  ...
  {
    console.log("[GeyMyturnCount] [Error] SELECT FAIL. DB Access or etc.:  " + " > " + error);
